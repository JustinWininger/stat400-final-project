...
# GLM Fitting and Analysis

```{r include = F}
int_only_model <- glm(HOF_ind ~ 1, family = binomial, data = qb_hof)

full_model <- glm(HOF_ind ~ seasons_played + pass_yards + Cmp + Att + TD + INT + first_downs + twenty_plus + forty_plus + Lng + Sck  + Decade, family = binomial, data = qb_hof)
...
full_model2 <- glm(HOF_ind ~ pass_yards + TD + INT + first_downs + Sck + seasons_played, family = binomial, data = qb_hof)

stats::step(object = full_model2, 
            scope = list(lower = int_only_model, upper = full_model2),
            data = qb_hof,
            direction = "backward")
```

```{r include = F}
logmodel <- glm(HOF_ind ~ TD + first_downs + seasons_played, family = binomial, data = qb_hof)

summary(logmodel)
```
```{r}
anova(logmodel, full_model2)
...
```{r message = F}
logmodel2 <- glm(HOF_ind ~ TD + first_downs + seasons_played, family = binomial, data = allqb_hof)

summary(logmodel2)

pred_prob2 <- predict(logmodel, newdata = allqb_hof, type = "response")
...
# generalizing to new data, accuracy of 0.9852
#Sensitivity : 0.9923         
#Specificity : 0.7692 
...
```
```{r}
 predictedHOFQBs <- allqb_hof %>% filter(Prediction2 =='Yes', HOF == 'No')
```
